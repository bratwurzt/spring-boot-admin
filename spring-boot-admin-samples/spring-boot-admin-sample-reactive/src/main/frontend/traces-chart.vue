...
<script>
  import {event as d3Event, area as d3Area, scaleLinear, scaleTime, extent, max, stack as d3Stack, axisBottom, axisRight, brushX, mouse, select} from "d3";
  ...
  export default {
    ...
    computed: {
      chartData() {
        ...
        for (let time = Math.floor(oldest.valueOf() / interval) * interval; time < now; time += interval) {
          ...
          while (idx >= 0 && this.traces[idx].timestamp.valueOf() < time + interval) {
            ...
            tracesInCurrentInterval.push(trace);
            idx--;
          }

          if(tracesInCurrentInterval.length){
            bucket.yAxisValue = !!this.chartValueFn ? this.chartValueFn(tracesInCurrentInterval) : tracesInCurrentInterval.length;
            bucket.traces = tracesInCurrentInterval;
          }
      ...
      tooltipContent() {
        ...
        const selectedBuckets = this.chartData.filter(
    ...
    methods: {
      ...
      drawChart(data) {
        ...
        const x = scaleTime()
          .range([0, vm.width])
          .domain(extent(data, d => d.timeStart));
        this.x = x;

        const y = scaleLinear()
          .range([vm.height, 0])
          .domain([0, max(data, d => d.yAxisValue)]);
        ...
        //draw axis
        vm.xAxis.call(axisBottom(x)
          .ticks(10)
          .tickFormat(d => moment(d).format('HH:mm:ss'))
        );

        vm.yAxis.call(axisRight(y)
          .ticks(Math.min(5, max(data, d => d.yAxisValue)))
          .tickSize(this.width)
        ).call(
        ...
        const brush = brushX()
          ...
          .on('brush', function () {
            ...
            if (d3Event.selection) {
              ...
              select(this).call(d3Event.target.move, [floor, ceil].map(x));
        ...
        vm.brushGroup.call(brush)
          .on('mousemove', () => {
            ...
            const mouseX = mouse(vm.brushGroup.select('.overlay').node())[0];
    ...
    mounted() {
      ...
      this.chartLayer = select(this.$el.querySelector('.trace-chart__svg'))
        .append('g')
